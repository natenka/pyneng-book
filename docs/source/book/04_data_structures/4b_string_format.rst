...
необходимо отформатировать вывод столбцами \* надо конвертировать числа
в двоичный формат

Существует два варианта форматирования строк: \* с оператором ``%``
(более старый вариант) \* методом ``format()`` (новый вариант)
...
\* ``%s`` - строка или любой другой объект в котором есть строковое
представление \* ``%d`` - integer \* ``%f`` - float
