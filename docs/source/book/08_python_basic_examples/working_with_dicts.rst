...
Пример вывода команды sh ip int br (файл sh\_ip\_int\_br.txt):
...
Файл working\_with\_dict\_example\_1.py:
...
имя интерфейса - MTU (файл sh\_ip\_interface.txt):
...
интерфейсом, а затем через несоколько строк - строка с MTU. Если
...
Файл working\_with\_dict\_example\_2.py:
...
в случае с выводом sh ip interface, могут попадаться интерфейс, которые
выглядят так:
...
результат будет таким (вывод для файла sh\_ip\_interface2.txt):
...
working\_with\_dict\_example\_4.py):
