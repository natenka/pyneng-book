...
Также внутри функции указано, что при подключению к устройству с адресом 192.168.100.1,
...
* ``result = executor.map(send_show, devices, repeat('sh clock'))`` - метод map похож на
  ...
  * так как вместо списка команд, тут используется только одна команда,
    ...
    разным устройства. Для этого используется функция repeat - она повторяет команду ровно столько
