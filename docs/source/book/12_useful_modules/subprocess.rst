...
| При этом он может подключаться к `стандартным потокам
ввода/вывода/ошибок <http://xgu.ru/wiki/stdin>`__ и получать код
возврата.
...
Обратите внимание, что если необходимо вызвать команду с аргументами, её
...
Пример использования модуля subprocess (файл subprocess\_run\_basic.py):
...
Функция ping\_ip проверяет доступность IP-адреса и возвращает True и
