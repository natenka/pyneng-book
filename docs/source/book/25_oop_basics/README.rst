...
текущего объекта. Например, переменная экземпляра conn.in\_transaction
...
conn.in\_transaction, возвращает True:
...
.. code:: python
    ...
    На примере со сторокой понятно, что класс не обязан хранить
...
разберемся с формальной терминлогией.
